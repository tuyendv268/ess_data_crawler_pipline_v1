...
def perform_clustering(embs_and_timestamps, AUDIO_RTTM_MAP, out_rttm_dir, clustering_params):
    ...
    for uniq_id, value in tqdm(AUDIO_RTTM_MAP.items(), desc='clustering', leave=False):
        ...
        # print("clustering_params.enhanced_count_thres: ", clustering_params.enhanced_count_thres)
        # print("clustering_params.max_rp_threshold: ", clustering_params.max_rp_threshold)
        # print("clustering_params.maj_vote_spk_count: ", clustering_params.maj_vote_spk_count)
        # print("clustering_params.sparse_search_volume: ", clustering_params.sparse_search_volume)
    ...
    with open('embs_and_timestamps.pkl', 'wb') as handle:
        pkl.dump(embs_and_timestamps, handle)
        print("saved: ", embs_and_timestamps)
    if out_rttm_dir:
    ...
    if len(all_reference) == len(all_hypothesis):
        ...
        logging.info(
            "Cumulative Results for collar {} sec and ignore_overlap {}: \n FA: {:.5f}\t MISS {:.5f}\t \
                Diarization ER: {:.5f}\t, Confusion ER:{:.5f}".format(
