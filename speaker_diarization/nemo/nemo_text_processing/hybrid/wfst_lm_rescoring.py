...
import model_utils
import pandas as pd
import segment_audio_by_duration
...
def _get_unchanged_count(text):
    ...
    text = segment_audio_by_duration.remove_punctuation(text, remove_spaces=False, do_lower=False, exclude=exclude)
    text_clean = ""
    for ch in text:
...
def main():
    ...
    print("LOAD DATA...")
    inputs, targets, _, _ = segment_audio_by_duration.load_data(input_fs)
    pre_inputs, pre_targets = segment_audio_by_duration.clean_pre_norm(dataset=args.dataset, inputs=inputs, targets=targets)
    ...
    post_targets, post_norm_texts_weights = segment_audio_by_duration.clean_post_norm(
        dataset=args.dataset, inputs=pre_inputs, targets=pre_targets, norm_texts=norm_texts_weights
    )

    print("GETTING LABELS...")
    labels = segment_audio_by_duration.get_labels(targets=post_targets, norm_texts_weights=post_norm_texts_weights)
    ...
    for i, example in tqdm(enumerate(zip(post_norm_texts_weights, labels))):
        ...
        df['sent'] = df['sent'].apply(lambda x: segment_audio_by_duration.remove_whitelist_boudaries(x))
        df["weights"] = data[1]
        ...
        if do_print:
            ...
            segment_audio_by_duration.print_df(df)
            print("-" * 80 + "\n")
